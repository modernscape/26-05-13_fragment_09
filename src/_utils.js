...
function _rotationPos(x1, y1, z1, azimuth, elevation) {
  ...
  const x = (x1 * Math.cos(theta) - y1 * Math.sin(theta)) * Math.cos(alpha) + z1 * Math.sin(alpha);
  const y = x1 * Math.sin(theta) + y1 * Math.cos(theta);
  const z = -(x1 * Math.cos(theta) - y1 * Math.sin(theta)) * Math.sin(alpha) + z1 * Math.cos(alpha);
